...
export const UserInSearchItem = ({ id, photoURL, username, firstName, lastName, onClick }: UserInSearchItemProps) => {
    ...
    return (
        ...
        >
            <div className="flex h-full min-h-[48px] w-min min-w-[48px] items-center justify-center">
                <Avatar
                    src={photoURL}
                    alt={concatenate(firstName, lastName)}
            ...
            <div className="flex flex-1 flex-col gap-1 overflow-hidden transition-none">
                ...
                <div className="flex">
                    <span className={classNames("mt-2.5 inline-block truncate align-baseline text-sm", isActive ? "text-[#303030] dark:text-[#FFFFFF]" : "text-[#4C4C4C] dark:text-[#AFAFAF]")}>@{username}</span>
                </div>
            </div>
        </div>
    );
