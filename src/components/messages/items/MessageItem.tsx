...
export const MessageItem: FC<MessageItemProps> = ({
    ...
    }, [activeDialogId, id, inView, isCurrentUser, isRead]);
    ...
    return (
        <>
            ...
            <motion.div
                ...
                animate="visible"
                exit="hidden"
                className={classNames("message-item duration-250 relative relative flex px-2 py-1 transition-colors", isObserved && "bg-[#98BDE750] dark:bg-[#416D9C50]", isCurrentUser && "flex-row-reverse 2xl:flex-row", className)}
                ref={ref}
                id={id}
            >
                <div
                    className={classNames("relative mt-auto flex select-none items-center justify-center", !showAvatar && "ml-[40px]")}>
                    {showAvatar && (
                        <Avatar
                            src={sender.photoURL}
                            alt="image"
                ...
                <div
                    className={classNames("duration-250 relative mx-2.5 rounded-lg p-2.5 transition-[background-color] sm:max-w-[400px] md:max-w-[400px] lg:max-w-[450px] xl:max-w-[700px] 2xl:max-w-4xl", file ? "pr-14" : "pr-20", isCurrentUser ? "bg-[#5B9BD965] dark:bg-[#304F74]" : "bg-white dark:bg-[#151F38]")}>
                    {file ? (
                        <div
                        ...
                        <span
                            className="whitespace-pre-wrap break-all text-sm text-[#333333] dark:text-white">{text}</span>
                    )}

                    <div className="absolute bottom-2 right-2 z-0 flex select-none items-center text-xs">
                        {isCurrentUser && <>{isRead ?
                            <DoubleCheckIcon
                                className="mr-1 h-4 w-4 fill-[#696969] stroke-0 dark:fill-[#9E9E9E]"/> :
